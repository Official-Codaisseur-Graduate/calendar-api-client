...
export default class Calendar extends Component {
  ...
  dayList = () => {
    ...
    for (let emptyCell = 0; emptyCell < this.firstDayOfMonth(); emptyCell++) {
      emptyCells.push(
        <td key={-emptyCell} className='calendar-day empty'>
          {''}
        </td>
      );
    }
    ...
    for (let day = 1; day <= this.props.dateObject.daysInMonth(); day++) {
      ...
      let currentDay =
        day === Number(this.props.dateObject.format('D')) &&
        currentMonth === this.props.dateObject.format('MMMM')
          ? 'today'
          : '';
        ...
        <div className='tail-datetime-calendar'>
          <div className='calendar-navi'>
            ...
            {!this.props.showMonthTable && (
              <span onClick={this.props.showMonth} className='calendar-label'>
